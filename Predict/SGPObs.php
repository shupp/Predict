...
class Predict_SGPObs
{
    ...
    public static function Calculate_RADec_and_Obs(
        $_time, Predict_Vector $pos, Predict_Vector $vel, Predict_Geodetic $geodetic, Predict_ObsAstro $obs_set)
    {
    ...
    /*                Pedro Ramon Escobal, pp. 401-402 */
        ...
        $obs = new Predict_ObsSet();

        self::Calculate_Obs($_time, $pos, $vel, $geodetic, $obs);
    ...
    /*  if( isFlagSet(VISIBLE_FLAG) )
        ...
        $el = $obs->el;
        $phi   = $geodetic->lat;
        $theta = Predict_Math::FMod2p(Predict_Time::ThetaG_JD($_time) + $geodetic->lon);
        $sin_theta = sin(theta);
        $cos_theta = cos(theta);
        $sin_phi = sin(phi);
        $cos_phi = cos(phi);
        ...
        $obs_set->dec = Predict_Math::ArcSin($Lz);  /* Declination (radians)*/
        $cos_delta = sqrt(1 - ($Lz * $Lz));
        $sin_alpha = $Ly / $cos_delta;
        $cos_alpha = $Lx / $cos_delta;
        $obs_set->ra = Predict_Math::AcTan($sin_alpha, $cos_alpha); /* Right Ascension (radians)*/
        $obs_set->ra = Predict_Math::FMod2p($obs_set->ra);
        /*}*/  /*if*/
